        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('pemilik.kosan.update', $kosan) }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Kosan <span class="text-red-500">*</span></label>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-1">Kota <span class="text-red-500">*</span></label>
                ...
                {{-- Card: Detail & Harga --}}
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <h3 class="font-semibold text-gray-800 text-base mb-4 pb-3 border-b border-gray-100">💰 Detail & Harga</h3>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Harga per Bulan (Rp) <span class="text-red-500">*</span></label>
                            <input type="number" name="harga_per_bulan" value="{{ old('harga_per_bulan', $kosan->harga_per_bulan) }}"
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Tipe Kosan <span class="text-red-500">*</span></label>
                            <select name="tipe" class="w-full px-4 py-2.5 rounded-lg border {{ $errors->has('tipe') ? 'border-red-400 bg-red-50' : 'border-gray-200' }} text-sm focus:outline-none focus:ring-2 focus:ring-green-400 focus:border-transparent bg-white">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Jumlah Kamar <span class="text-red-500">*</span></label>
                            <input type="number" name="jumlah_kamar" value="{{ old('jumlah_kamar', $kosan->jumlah_kamar) }}" min="1"
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Kamar Tersedia <span class="text-red-500">*</span></label>
                            <input type="number" name="kamar_tersedia" value="{{ old('kamar_tersedia', $kosan->kamar_tersedia) }}" min="0"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <select name="status" class="w-full px-4 py-2.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-green-400 bg-white">
                ...
                {{-- Card: Fasilitas --}}
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <h3 class="font-semibold text-gray-800 text-base mb-4 pb-3 border-b border-gray-100">✅ Fasilitas</h3>
                    @php
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        @foreach($fasilitasList as $fasilitas)
                            <label class="flex items-center gap-2 cursor-pointer group">
                                ...
                                <span class="text-sm text-gray-700 group-hover:text-green-700 transition">{{ $fasilitas }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                ...
                @if($kosan->fotos->count())
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        <h3 class="font-semibold text-gray-800 text-base mb-4 pb-3 border-b border-gray-100">🖼️ Foto Saat Ini</h3>
                        <div class="grid grid-cols-3 sm:grid-cols-4 gap-3">
                            @foreach($kosan->fotos as $foto)
                                <div class="relative group">
                                    <img src="{{ asset('storage/' . $foto->foto) }}" class="w-full h-24 object-cover rounded-lg border border-gray-100">
                                    @if($foto->is_utama)
                                        <span class="absolute top-1 left-1 bg-green-600 text-white text-xs px-1.5 py-0.5 rounded font-semibold">Utama</span>
                                    @endif
                                </div>
                            @endforeach
                        </div>
                        <p class="text-xs text-gray-400 mt-3">Upload foto baru di bawah akan ditambahkan ke foto yang sudah ada.</p>
                    </div>
                @endif

                {{-- Card: Upload Foto Baru --}}
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <h3 class="font-semibold text-gray-800 text-base mb-1 pb-3 border-b border-gray-100">📷 Tambah Foto Baru</h3>
                    <p class="text-xs text-gray-400 mt-3 mb-4">Opsional. Format: JPG, PNG, GIF. Maks 2MB/foto.</p>
